...
def fix_chapter_33():
    ...
    # 提取正確的章節標題
    # 從<title>標籤提取，移除多餘部分
    title_match = re.search(r'<title>第33章：([^<]+?) - 科技修真傳</title>', content)
    if title_match:
        chapter_title = title_match.group(1).strip()
    else:
        # 嘗試從<h2>標籤提取
        h2_match = re.search(r'<h2 class="reader-chapter-title">第33章：([^<]+)</h2>', content)
        if h2_match:
            chapter_title = h2_match.group(1).strip()
        else:
            chapter_title = "修煉之路"
    
    print(f"章節標題: {chapter_title}")
    
    # 提取<div class="chapter">中的內容
    ...
    for line in lines:
        ...
        # 檢查是否為小節標題（格式：<p>## 第一節：標題</p>）
        section_match = re.match(r'<p>##\s*(第[一二三四五六七八九十\d]+節：\s*[^<]+)</p>', line)
        if section_match:
            # 如果已經在收集一個小節，先保存它
            if collecting and current_section['paragraphs']:
                sections.append(current_section.copy())
            
            # 提取小節標題，移除"第X節："前綴
            full_title = section_match.group(1).strip()
            # 移除"第X節："，保留冒號後面的標題
            title = re.sub(r'^第[一二三四五六七八九十\d]+節：\s*', '', full_title)
            
            current_section = {'title': title, 'paragraphs': []}
            collecting = True
            continue
        
        # 如果是章節標題行（<p># 《科技修真傳》第33章</p>），跳過它
        if re.match(r'<p>#\s*《科技修真傳》第\d+章</p>', line):
            continue
        
        # 處理段落內容
        if line.startswith('<p>') and collecting:
            # 移除<p>和</p>標籤
    ...
    # 如果沒有找到小節，可能是格式問題
    if not sections:
        print("警告: 未找到小節，嘗試替代解析方法...")
        # 嘗試將所有段落作為一個小節
        paragraphs = []
        for line in lines:
            line = line.strip()
            if line.startswith('<p>') and not re.match(r'<p>#', line):
        ...
        if paragraphs:
            ...
            print(f"  將所有內容作為一個小節 ({len(paragraphs)} 段落)")
...
{paragraphs_html}
    ...
    try:
        with open('chapter-34.html', 'r', encoding='utf-8'):
